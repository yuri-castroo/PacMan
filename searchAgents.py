...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        self.cornerFood = tuple(self.cornerFood)
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                for corner in cornerState:
                    position = corner[0]
                    if position == nextPos:
                        newState.append((position, True))
                    else:
                        newState.append((position, corner[1]))
                newState = tuple(newState)
                successors.append(((nextPos, newState), action, 1))
...
def cornersHeuristic(state, problem):
    ...
    for corner, state in cornerState:
        if not state:
            currMax = max(currMax, distance(corner, position))
    return currMax
...
def foodHeuristic(state, problem):
    ...
    #save food as a list to iterate through
    max = 0
    for dot in food:
        ...
        while not queue.isEmpty():
            ...
            for succ in successors:
                ...
                if succState not in visited:
                    queue.push((succState, directions +  [succDir]))
